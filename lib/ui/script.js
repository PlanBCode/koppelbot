function XYZ() {
    ...
    function Property(propertyName, meta) {
        ...
        this.createCreator = options => {
            ...
            if (xyz.types.hasOwnProperty(type) && xyz.types[type].hasOwnProperty('edit')) {
                const uri = ''; //TODO
                const content = '';  //TODO default
                // TODO html label for gebruiken
                html += `<tr><td>${propertyName}</td><td>
                    ${xyz.types[type].edit(uri, content, settings, options)}
                    </td></tr>`;
            } else if (!primitive) {
                for (let propertyName in subProperties) {
                    html += subProperties[propertyName].createCreator(options);
                }
            }
            return html;
        };
    ...
    function Entity(entityClass, meta) {
        ...
        this.createCreator = options => {
            ...
            for (let propertyName in properties) {
                html += properties[propertyName].createCreator(options);
            }
            html += '</table>';
            return html;
        }
    ...
    this.create = (uri, options) => {
        ...
        retrieveMeta(uri, () => {
            ...
            const entityClass = entityClasses[entityClassName];
            TAG.innerHTML = entityClass.createCreator(options)+'<input type="submit"/>';
        });
    }
