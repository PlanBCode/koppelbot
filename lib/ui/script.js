function XYZ() {
    ...
    function Property(meta) {
        ...
        if (meta.hasOwnProperty('signature')) {
            primitive = false;
            for (let propertyName in meta.signature) {
                subProperties[propertyName] = new Property(meta.signature[propertyName]);
            }
        }
    ...
    function Entity(entityClass, meta) {
        ...
        for (let propertyName in meta) {
            properties[propertyName] = new Property(meta[propertyName]);
        }
    ...
    const renderNode = (action, options) => (content, uri, status,) => {
        ...
        if (this.types.hasOwnProperty(type)) {
            if (this.types[type].hasOwnProperty(action)) {
                return this.types[type][action](uri, content, settings, options);
                ...
                for (let subPropertyName in settings.signature) {
                    ...
                    html += renderNode(action, uri + '/' + subPropertyName, subSettings, options)(status, content[subPropertyName]);
                }
                return html;
            } else {
    ...
    this.ui = (uri, options) => {
        ...
        registerUri(uri, renderDisplay(TAG, options));
    };
