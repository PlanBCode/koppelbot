...
const expand = (action, meta, data, uri, options) => {
    ...
    for (let propertyName in meta) {
        ...
        if (settings.hasOwnProperty('type')) { //TODO this can collide with a property named 'type' SEE collapse
            dom[propertyName] = renderEntity(action, subUri, content, settings, options);
        } else {
...
function DataNode(status, uri, response, meta) {

    //TODO use response and status to determine data
    this.subDataNodes = {};
    this.status = status;
    this.uri = uri;
    this.data = data;
    ...
    if (meta.hasOwnProperty('type')) {
        ...
        for (let propertyName in meta) {//TODO this can collide with a property named 'type' SEE collapse, expand
            const subMeta = meta[propertyName];
            const suData = data[propertyName];
            const subUri = uri + '/' + propertyName;
            this.subDataNodes[propertyName] = new DataNode(status, subUri, subData, subMeta);
        }
    }
}
...
const renderDisplay = (DIV, options) => (uri) => {
    ...
    request('GET', uri + '?meta', undefined, (meta,status) => {//TODO add querystring better
        meta = JSON.parse(meta);
        request('GET', uri, undefined, (data,status) => {//TODO add querystring better
            const dom = {};
