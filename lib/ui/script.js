...
function list(DIV, options, data, meta, dom) {
    ...
    for (let entityClass in data) {
        ...
        for (let entityId in data[entityClass]) {
            ...
            //TODO loop through subproperties
            for (let propertyName in data[entityClass][entityId]) {
                const TD = document.createElement('TD');
...
const expand = (action,meta,data, uri, options) => {
    ...
    for (let propertyName in meta) {
        ...
        if(settings.hasOwnProperty('type')){ //TODO this can collide with a property named 'type'
            dom[propertyName] = renderEntity(action, uri, content, settings, options);
        }else{
            dom[propertyName]=expand(action,settings.combined,content,subUri,options);
        }
    }
    return dom;
};
