function XYZ() {
    ...
    function Property(propertyName, meta) {
        ...
        this.createCreator = options => {
            ...
            if (xyz.types.hasOwnProperty(type) && xyz.types[type].hasOwnProperty('edit')) {
                ...
                TR.appendChild(TD_label);
                const element = xyz.types[type].edit(uri, content, settings, options);
                const TD_content = document.createElement('TD');
                ...
                for (let propertyName in subProperties) {
                    TRs.push(...subProperties[propertyName].createCreator(options));
                }
            }
            return TRs;
        };
    ...
    function Entity(entityClass, meta) {
        ...
        this.createCreator = options => {
            ...
            for (let propertyName in properties) {
                for (let TR of properties[propertyName].createCreator(options)) {
                    TABLE.appendChild(TR);
                }
            }
            return TABLE;
    ...
    this.create = (uri, options) => {
        ...
        retrieveMeta(uri, () => {
            ...
            const entityClass = entityClasses[entityClassName];
            const TABLE = entityClass.createCreator(options);
