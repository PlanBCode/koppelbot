...
function renderElement(xyz, action, uri, status, content, settings, options) {
    ...
    if (types[type].hasOwnProperty(action)) {
        ...
        let TAG = types[type][action](xyz, uri, status, content, settings, options, onChange);
        ...
        const listeners = xyz.on(uri, 'changed', (entityId, node) => {
            ...
            if(PARENT) {
                ...
                TAG = types[type][action](xyz, uri, newStatus, newContent, settings, options, onChange);
