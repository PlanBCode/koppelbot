...
function EntityClass(entityClassName, settings) {
        ...
        if (path.length === 0) {
            ...
            for (let propertyName of propertyNames) {
                if (properties.hasOwnProperty(propertyName)) {
                    properties[propertyName].addPropertyListener(entityId, subPath, eventName, callback);
                } else {
                    //TODO throw error?
    ...
    this.addListener = (path, eventName, callback) => {
        ...
        for (let entityId of entityIds) {
            addEntityListener(entityId, subPath, eventName, callback);
        }
    };
