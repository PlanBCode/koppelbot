...
const Response2 = require('./response.js').Reponse;
    ...
    this.getEntityIdResponse = (path, entityId) => {
        ...
        for (let propertyName of propertyNames) {
            if (properties.hasOwnProperty(propertyName)) {
                ...
                response[propertyName] = new Response2(400, null, [`${propertyName} does not exist.`]); //TODO
            }
        }
        return response;
    };
    ...
    this.getResponse = path => {
        ...
        for (let entityId of entityIds) {
            if (entities.hasOwnProperty(entityId)) {
                ...
                response[entityId] = new Response2(404, null, [`/${entityClassName}/${entityId} not found.`]); //TODO
            }
        }
        return response;
    };
        ...
        if (entityStatus === 207) {
            for (let propertyName in properties) {
                ...
                if (property207Wrapper === null || typeof property207Wrapper !== 'object'
                    ...
                    const propertyStatus = property207Wrapper.status;
                    const propertyContent = property207Wrapper.content;
                    const propertyState = property.handleInput(entityId, propertyStatus, propertyContent, isDelta);
            ...
            for (let propertyName in properties) {
                ...
                const propertyContent = (entityContent === null || typeof entityContent !== 'object')
                    ? null
                    : entityContent[propertyName];
                const propertyState = property.handleInput(entityId, entityStatus, propertyContent, isDelta);
                state.addSubState(propertyState);
            }
        }
...
function getResponse(uri, entityClasses) {
    ...
    for (let entityClassName of entityClassNames) {
        if (entityClasses.hasOwnProperty(entityClassName)) {
            ...
            response[entityClassName] = new Response2(404, null, [`/${entityClassName} not found.`]); //TODO
        }
    }
    return response;
}
