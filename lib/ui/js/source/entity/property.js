...
exports.constructor = function Property(parent, propertyName, meta) {
    ...
    this.getResponse = (path, entityId) => {
        if (isPrimitive) {
            return new Response2(statusses[entityId], contents[entityId], errors[entityId]);
        } else {
            ...
            for (let subPropertyName of subPropertyNames) {
                if (subProperties.hasOwnProperty(subPropertyName)) {
                    ...
                    response[subPropertyName] = new Response2(400, null, [`${subPropertyName} does not exist.`]); //TODO
                }
            }
            return response;
        }
