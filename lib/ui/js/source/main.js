...
function XYZ() {
    ...
    const createResponse = (uri, status, content) => {
        //TODO check status
    ...
    const renderNode = (action, options) => (source, property, entityId) => {
        ...
        if (types.hasOwnProperty(type)) {
            ...
            if (types[type].hasOwnProperty(action)) {
                ...
                return types[type][action](this, uri, content, settings, options, onChange);
                ...
                for (let subPropertyName in settings.signature) {
                    ...
                    const element = renderNode(action, options, subSettings)(content[subPropertyName], uri + '/' + subPropertyName, status);
    ...
    const renderDisplay = (TAG, options) => (uri) => {
        this.get(uri, response => {
            ...
            const node = transform(response, renderNode(action, options));
    ...
    this.get = (uri, callback) => {
        ...
        retrieveMeta(uri, () => {
            ...
            request('GET', uri, undefined, (status, content) => {//TODO add querystring better
                const response = createResponse(uri, status, content);
                callback(response);
            });
        });
    };
