...
const displays = {
    list: require('./display/list.js').display,
...
function XYZ() {
    ...
    const renderDisplay = (uri, options, WRAPPER) => (entityId, node) => {
        const displayName = options.display ||  DEFAULT_DISPLAY ;
        ...
        if (WRAPPER.classList.contains('xyz-empty') || WRAPPER.classList.contains('xyz-waiting')) {
            ...
            if (display && display.hasOwnProperty('empty')) {
                display.empty(this, options, WRAPPER, entityClassNameList);
            } else {
                WRAPPER.innerHTML = 'Empty'; //TODO
            }

            if (display && display.hasOwnProperty('first')) {
                display.first(this, options, WRAPPER, entityId, node);
            } else {
                WRAPPER.innerHTML = '';
            }
        }
        ...
        if (display && display.hasOwnProperty('entity')) {
            display.entity(this, options, WRAPPER, entityId, node);
        } else {
        ...
        registerUri(uri, uri => {
                this.get(uri, () => {
                    if (WRAPPER.classList.contains('xyz-waiting')) {
                        ...
                        if (display && display.hasOwnProperty('empty')) {
                            display.empty(this, options, WRAPPER);
                        } else {
                    ...
                    this.on(baseUri, 'available', renderDisplay(uri, options, WRAPPER));
                });
            },
            () => {
                WRAPPER.classList.add('xyz-waiting');
                if (display && display.hasOwnProperty('waiting')) {
                    display.waiting(this, options, WRAPPER);
                } else {
