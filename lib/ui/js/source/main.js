...
function XYZ() {
    ...
    const renderDisplay2 = (uri, options, TAG) => (entityId, eventName) => {
        const action = options.action || DEFAULT_ACTION;
        const display = options.display || DEFAULT_DISPLAY;
        if (TAG.classList.contains('xyz-empty')) {
            TAG.classList.remove('xyz-empty');
            TAG.innerHTML = ''; //TODO
        }else if (TAG.classList.contains('xyz-waiting')) {
            TAG.classList.remove('xyz-waiting');
            TAG.innerHTML = ''; //TODO
        }
        //TODO create

        TAG.innerHTML += 'X'; //TODO
        console.log('hello', entityId, eventName);
        console.log(entity.getResponse(uri, entityClasses));
    };

    const renderUiElement2 = (uri, options, TAG) => { //TODO rename
        this.get(uri, ()=>{
            if (TAG.classList.contains('xyz-waiting')) {
                TAG.classList.remove('xyz-waiting');
                TAG.classList.add('xyz-empty');
                TAG.innerHTML = 'No items available'; //TODO
            }
        }, () => {
            TAG.classList.add('xyz-waiting');
            TAG.innerHTML = 'Not ready';
            const baseUri = uriTools.getBaseUri(uri);
            this.on(baseUri, 'created', renderDisplay2(uri, options, TAG));
        });
    };

    this.ui2 = (uri, options, PARENT) => {//TODO rename
        options = options || {};
        let SCRIPT;
        if (typeof PARENT === 'undefined') {
            const tag = options.tag || DEFAULT_TAG;
        ...
        if (options.display === 'create') {
            ...
            renderUiElement2(uri, options, PARENT);//TODO rename
        }
        return PARENT;
    };
}
